...
function Menghub:MakeNotify(NotifyConfig)
    ...
    -- Warna bar progress (ungu muda)
    local BAR_COLOR = Color3.fromRGB(190, 150, 250)
    local PADDING_X = 14 -- padding kiri+kanan di sekitar teks
    local MIN_WIDTH = 120
    local MAX_WIDTH = 400
    local CONTAINER_WIDTH = 420
    local TOP_OFFSET = IsMobile and 60 or 95 -- jarak dari atas layar, sesuaikan biar gak numpuk sama HUD
    ...
    spawn(function()
        ...
        -- Layout container: nempel kanan-ATAS, stack ke bawah
        if not CoreGui.NotifyGui:FindFirstChild("NotifyLayout") then
            local NotifyLayout = Instance.new("Frame")
            NotifyLayout.AnchorPoint = Vector2.new(1, 0)
            NotifyLayout.BackgroundTransparency = 1
            NotifyLayout.BorderSizePixel = 0
            NotifyLayout.Position = UDim2.new(1, -20, 0, TOP_OFFSET)
            NotifyLayout.Size = UDim2.new(0, CONTAINER_WIDTH, 1, -TOP_OFFSET)
            NotifyLayout.Name = "NotifyLayout"
            NotifyLayout.Parent = CoreGui.NotifyGui

            CoreGui.NotifyGui.NotifyLayout.ChildRemoved:Connect(function()
                -- Kumpulkan sisa notif, urutkan dari yang paling atas
                local Remaining = {}
                ...
                for _, v in ipairs(Remaining) do
                    TweenService:Create(
                        ...
                        { Position = UDim2.new(1, 0, 0, StackY) }
                    ):Play()
                    StackY = StackY + v.Size.Y.Offset + 8
                end
            end)
        end

        -- Hitung posisi Y berdasarkan tinggi notif yang sudah ada
        local NotifyPosY = 0
        for i, v in CoreGui.NotifyGui.NotifyLayout:GetChildren() do
            if v:IsA("Frame") and v.Name == "NotifyFrame" then
                NotifyPosY = NotifyPosY + v.Size.Y.Offset + 8
            end
        end
        ...
        -- Reservasi slot & posisi SEKARANG JUGA (synchronous), sebelum ada kemungkinan yield
        -- di bawah (GetTextBoundsAsync). Ini yang mencegah notif "kebalap"/nge-gap kalau
        -- banyak dipanggil hampir bersamaan.
        ...
        -- Hitung lebar box berdasarkan panjang teks judul (boleh yield, slot udah aman)
        local BoundsParams = Instance.new("GetTextBoundsParams")
        BoundsParams.Text = NotifyConfig.Title
        BoundsParams.Font = Font.new(Enum.Font.GothamBold.Name, Enum.FontWeight.Bold)
        BoundsParams.Size = 12
        BoundsParams.Width = MAX_WIDTH

        local TextWidth
        local Success, Result = pcall(function()
            return TextService:GetTextBoundsAsync(BoundsParams)
        end)
        if Success and Result then
            TextWidth = Result.X
        else
            -- Fallback: estimasi manual (~6.5px per karakter buat GothamBold size 12)
            TextWidth = #NotifyConfig.Title * 6.5
        end

        local BoxWidth = math.clamp(TextWidth + PADDING_X * 2, MIN_WIDTH, MAX_WIDTH)
        NotifyFrame.Size = UDim2.new(0, BoxWidth, 0, 36)

        -- Panel utama: gelap, sudut membulat kecil
        ...
        NotifyFrameReal.Position = UDim2.new(0, BoxWidth + 20, 0, 0)
        ...
        ProgressBack.BackgroundTransparency = 0.85
        ProgressBack.BorderSizePixel = 0
        ProgressBack.AnchorPoint = Vector2.new(0.5, 1)
        ProgressBack.Position = UDim2.new(0.5, 0, 1, -6)
        ProgressBack.Size = UDim2.new(1, -14, 0, 3)
        ProgressBack.Name = "ProgressBack"
        ProgressBack.Parent = NotifyFrameReal
        ...
        -- Bar berkurang secara linear selama durasi Delay
        TweenService:Create(
            ProgressBar,
            TweenInfo.new(tonumber(NotifyConfig.Delay), Enum.EasingStyle.Linear, Enum.EasingDirection.Out),
...
function notif(msg, delay, color, title, desc)
    return Menghub:MakeNotify({
        Title = msg or title or "Meng Hub",
        Delay = delay or 4
    })
end
